...
// The LangChain track has its own spine: each layer names the last section that belongs to it.
const TRACK_LAYERS: [string, number][] = [['Model', 1], ['Tools + agent', 4], ['Output + memory', 7], ['Knowledge + planning', 9], ['Guardrails + approval', 11], ['Graphs + specialists', 13], ['Production', 14]];
const TITLE_PREFIX = /^(?:Day \d+(?:\.\d+| Project| Capstone)?|LangChain L\d+) — /;
...
function DayStory({ day, lessonOrder }: { day: CourseDay; lessonOrder: number }) {
  ...
  return <section className="day-story"><div><p className="eyebrow">{day.label} · what you build {day.kind === 'day' ? 'today' : 'in this track'}</p><h2>{day.project}</h2><p>{day.projectBrief}</p><p className="project-signpost">The pieces come together in section {finalSection}. You are not expected to understand the whole project before then.</p></div><ol>{day.projectFlow.map((stage, index) => <li key={stage} className={lessonOrder >= Math.ceil(((index + 1) / day.projectFlow.length) * day.projectLesson) ? 'reached' : ''}><span>{index + 1}</span>{stage}</li>)}</ol></section>;
}

function CourseSpine({ day, lesson }: { day: CourseDay; lesson: Notebook }) {
  if (day.kind === 'track') {
    const activeIndex = TRACK_LAYERS.findIndex(([, lastSection]) => lesson.order <= lastSection);
    return <div className="course-spine">{TRACK_LAYERS.map(([layer], index) => <div key={layer} className={index === activeIndex ? 'active' : ''}><span>L</span>{layer}{index < TRACK_LAYERS.length - 1 && <ChevronRight />}</div>)}</div>;
  }
  ...
  return <div className="lesson-guide getting-started">
    <section className="start-hero"><p className="eyebrow">Start here</p><h2>How this course works</h2>
      <ul className="start-list">
        ...
        <li><strong>Plus a separate LangChain track.</strong> One more notebook, outside the day sequence, grows a single agent fourteen times with LangChain 1.x and LangGraph. Take it after Day 1 or on its own.</li>
    ...
    <section className="course-map"><h2>The LangChain track</h2><p className="section-hint">A separate notebook that teaches the same ideas through LangChain 1.x and LangGraph. Click it to open section L1.</p>
  ...
  return <SidebarProvider style={{ '--sidebar-width': '23rem', '--sidebar-width-icon': '4.5rem' } as CSSProperties}>
    <Sidebar className="border-r border-white/10 bg-[var(--ink)] text-white" collapsible="icon">
      ...
      <SidebarContent>
        ...
        <SidebarGroup><SidebarGroupLabel className="px-3 text-[11px] uppercase tracking-[.16em] text-white/40 group-data-[collapsible=icon]:hidden">Separate track</SidebarGroupLabel><SidebarGroupContent className="space-y-1 px-2">
    ...
    <SidebarInset className="min-w-0 bg-[var(--paper)]">
      {view === 'start' ? <>
        ...
        <article className="mx-auto w-full max-w-5xl px-5 py-8 sm:px-9 sm:py-10"><div className="mb-7 border-b border-[var(--line)] pb-7"><div className="mb-4 flex flex-wrap items-center gap-2"><Badge style={{ backgroundColor: activeDay.color, color: '#10212b' }}>{activeDay.label}</Badge>{activeNotebook.isExercise && <Badge variant="outline">Hands-on exercise</Badge>}{activeNotebook.isProject && <Badge variant="outline">Project</Badge>}<div className="top-lesson-nav"><Button variant="outline" size="sm" disabled={!previousLesson} onClick={() => previousLesson && selectNotebook(previousLesson.day, previousLesson.notebook)} aria-label="Previous section"><ArrowLeft />Previous</Button><span>{activeIndex + 1} of {allNotebooks.length}</span><Button variant="outline" size="sm" disabled={!nextLesson} onClick={() => nextLesson && selectNotebook(nextLesson.day, nextLesson.notebook)} aria-label="Next section">Next<ArrowRight /></Button></div></div><h1 className="max-w-4xl text-3xl font-bold tracking-[-.035em] text-[var(--ink)] sm:text-4xl">{shortTitle(activeNotebook.title)}</h1><p className="mt-3 max-w-3xl text-base leading-7 text-[var(--muted-ink)]">{activeNotebook.description}</p><div className="mt-5 flex flex-wrap items-center gap-2 text-sm text-[var(--muted-ink)]"><span className="meta-pill"><BookOpen />Explanations included</span><span className="meta-pill"><Code2 />{activeNotebook.codeCells} code cells</span><span className="meta-pill"><Network />{activeNotebook.diagrams.length} diagram{activeNotebook.diagrams.length === 1 ? '' : 's'}</span>{activeNotebook.hasLiveObservation && <span className="meta-pill"><Sparkles />Live observation</span>}</div></div>
          <Tabs defaultValue="lesson" key={activeNotebook.id}><div className="lesson-toolbar"><TabsList className="lesson-tabs"><TabsTrigger value="lesson"><BookOpen />Learn</TabsTrigger><TabsTrigger value="code"><Code2 />Notebook</TabsTrigger><TabsTrigger value="architecture"><Network />System view</TabsTrigger></TabsList><div className="lesson-actions">{colabLink(dayColabUrl, `Open ${activeDay.name} notebook in Colab`)}</div></div>
            ...
            <TabsContent value="architecture" className="mt-8"><div className="mb-7"><h2 className="text-2xl font-bold">Where this section fits</h2><p className="mt-2 max-w-3xl text-[var(--muted-ink)]">Find {activeDay.kind === 'day' ? "today's layer in the course spine" : "this section's layer in the track spine"}, then follow the data and control flow for this section.</p><CourseSpine day={activeDay} lesson={activeNotebook} /></div><div className="space-y-6">{activeNotebook.diagrams.map((diagram) => <MermaidDiagram key={diagram.id} diagram={diagram} />)}</div></TabsContent>
          </Tabs>
          <footer className="mt-12 rounded-2xl border border-[var(--line)] bg-white p-5 shadow-sm"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="font-semibold">Make it yours</p><p className="mt-1 text-sm text-[var(--muted-ink)]">Change one input in the notebook, run the cell again, and explain what changed and why.</p></div><div className="flex flex-wrap gap-2"><Button variant={completed.includes(activeNotebook.id) ? 'secondary' : 'outline'} onClick={toggleComplete}>{completed.includes(activeNotebook.id) ? <Check /> : <Circle />}{completed.includes(activeNotebook.id) ? 'Completed' : 'Mark complete'}</Button></div></div></footer>
