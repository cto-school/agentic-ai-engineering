...
function DayStory({ day, lessonOrder }: { day: CourseDay; lessonOrder: number }) {
  return <section className="day-story"><div><p className="eyebrow">Day {day.number} · what you build today</p><h2>{day.project}</h2><p>{day.projectBrief}</p><p className="project-signpost">The pieces come together in section {day.number}.{day.projectLesson}. You are not expected to understand the whole project before then.</p></div><ol>{day.projectFlow.map((stage, index) => <li key={stage} className={lessonOrder >= Math.ceil(((index + 1) / day.projectFlow.length) * day.projectLesson) ? 'reached' : ''}><span>{index + 1}</span>{stage}</li>)}</ol></section>;
}
  ...
  return <SidebarProvider style={{ '--sidebar-width': '23rem', '--sidebar-width-icon': '4.5rem' } as CSSProperties}>
    ...
    <SidebarInset className="min-w-0 bg-[var(--paper)]">
      {view === 'start' ? <>
        ...
        <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-[var(--line)] bg-[color:var(--paper)]/92 px-4 backdrop-blur-xl sm:px-7"><SidebarTrigger><Menu /></SidebarTrigger><div className="min-w-0"><p className="truncate text-sm font-semibold">Day {activeDay.number} · {activeDay.title}</p><p className="truncate text-xs text-[var(--muted-ink)]">Section {activeDay.number}.{activeNotebook.order} of {activeDay.notebooks.length}</p></div><div className="ml-auto flex items-center gap-2">{colabReady && <a className="secondary-link hidden sm:inline-flex" href={githubUrl(colab)} target="_blank" rel="noreferrer"><GitBranch />GitHub</a>}{colabLink(dayColabUrl, `Open Day ${activeDay.number} in Colab`)}</div></header>

        <article className="mx-auto w-full max-w-5xl px-5 py-8 sm:px-9 sm:py-10"><div className="mb-7 border-b border-[var(--line)] pb-7"><div className="mb-4 flex flex-wrap items-center gap-2"><Badge style={{ backgroundColor: activeDay.color, color: '#10212b' }}>Day {activeDay.number}</Badge>{activeNotebook.isExercise && <Badge variant="outline">Hands-on exercise</Badge>}{activeNotebook.isProject && <Badge variant="outline">Project</Badge>}<div className="top-lesson-nav"><Button variant="outline" size="sm" disabled={!previousLesson} onClick={() => previousLesson && selectNotebook(previousLesson.day, previousLesson.notebook)} aria-label="Previous section"><ArrowLeft />Previous</Button><span>{activeIndex + 1} of {allNotebooks.length}</span><Button variant="outline" size="sm" disabled={!nextLesson} onClick={() => nextLesson && selectNotebook(nextLesson.day, nextLesson.notebook)} aria-label="Next section">Next<ArrowRight /></Button></div></div><h1 className="max-w-4xl text-3xl font-bold tracking-[-.035em] text-[var(--ink)] sm:text-4xl">{shortTitle(activeNotebook.title)}</h1><p className="mt-3 max-w-3xl text-base leading-7 text-[var(--muted-ink)]">{activeNotebook.description}</p><div className="mt-5 flex flex-wrap items-center gap-2 text-sm text-[var(--muted-ink)]"><span className="meta-pill"><BookOpen />Explanations included</span><span className="meta-pill"><Code2 />{activeNotebook.codeCells} code cells</span><span className="meta-pill"><Network />{activeNotebook.diagrams.length} diagram{activeNotebook.diagrams.length === 1 ? '' : 's'}</span>{activeNotebook.hasLiveObservation && <span className="meta-pill"><Sparkles />Live observation</span>}</div></div>
          <Tabs defaultValue="lesson" key={activeNotebook.id}><div className="lesson-toolbar"><TabsList className="lesson-tabs"><TabsTrigger value="lesson"><BookOpen />Learn</TabsTrigger><TabsTrigger value="code"><Code2 />Notebook</TabsTrigger><TabsTrigger value="architecture"><Network />System view</TabsTrigger></TabsList><div className="lesson-actions"><a className="secondary-link" href={activeDay.masterPublicPath} target="_blank" rel="noreferrer"><FileJson />Download the day</a>{colabLink(lessonColabUrl, 'Open this section only')}</div></div>
            ...
            <TabsContent value="code" className="mt-8"><div className="mb-6"><h2 className="text-2xl font-bold">The notebook, cell by cell</h2><p className="mt-2 max-w-3xl text-[var(--muted-ink)]">Every explanation and every code cell of this section, in the order you run them. Definitions from earlier sections carry forward, so run the day notebook from the top rather than this section alone.</p><div className="mt-4 flex flex-wrap gap-2">{colabLink(dayColabUrl, `Open Day ${activeDay.number} in Colab`, false)}<a className="secondary-link" href={activeNotebook.publicPath} target="_blank" rel="noreferrer"><FileJson />Download this section</a></div></div><NotebookWalkthrough notebook={activeNotebook} />{!activeNotebook.codeCells && <p className="rounded-xl border border-dashed p-6 text-[var(--muted-ink)]">This section is conceptual and has no code cells.</p>}</TabsContent>
            <TabsContent value="architecture" className="mt-8"><div className="mb-7"><h2 className="text-2xl font-bold">Where this section fits</h2><p className="mt-2 max-w-3xl text-[var(--muted-ink)]">Find today&apos;s layer in the course spine, then follow the data and control flow for this section.</p><div className="course-spine">{COURSE_LAYERS.map((layer, index) => <div key={layer} className={index + 1 === activeDay.number ? 'active' : ''}><span>{index + 1}</span>{layer}{index < COURSE_LAYERS.length - 1 && <ChevronRight />}</div>)}</div></div><div className="space-y-6">{activeNotebook.diagrams.map((diagram) => <MermaidDiagram key={diagram.id} diagram={diagram} />)}</div></TabsContent>
          </Tabs>
          <footer className="mt-12 rounded-2xl border border-[var(--line)] bg-white p-5 shadow-sm"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="font-semibold">Make it yours</p><p className="mt-1 text-sm text-[var(--muted-ink)]">Change one input in the notebook, run the cell again, and explain what changed and why.</p></div><div className="flex flex-wrap gap-2"><a className="secondary-link" href={activeNotebook.publicPath} target="_blank" rel="noreferrer"><FileJson />Download this section</a>{colabLink(dayColabUrl, `Open Day ${activeDay.number} in Colab`)}<Button variant={completed.includes(activeNotebook.id) ? 'secondary' : 'outline'} onClick={toggleComplete}>{completed.includes(activeNotebook.id) ? <Check /> : <Circle />}{completed.includes(activeNotebook.id) ? 'Completed' : 'Mark complete'}</Button></div></div></footer>
