...
def main() -> int:
    ...
    checks = {
        ...
        "three corpus documents": len(list((ROOT / "data" / "corpus").glob("*.md"))) == 3,
        "ten golden cases": len(json.loads((ROOT / "data" / "golden_set.json").read_text(encoding="utf-8"))) == 10,
    }
    failed = False
    print(f"DAY2_MODE={mode}")
    for label, passed in checks.items():
        classroom_only = label.endswith("(classroom)") or "(classroom)" in label
